...
title = st.empty()
res_place = st.empty()
res = st.session_state.all_data["CPSETF"]
...
while True:
    if time.time() - st.session_state.last_analysis_time >= 10:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['CPSETF']),2)
